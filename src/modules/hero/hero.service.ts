...
class HeroService extends BaseService<IHeroDocument, typeof heroRepository> {
  ...
  async createHero(dto: CreateHeroDto, file?: Express.Multer.File): Promise<IHeroDocument> {
    const payload: Partial<IHeroDocument> = { ...dto } as unknown as Partial<IHeroDocument>;
    const existing = await this.getHero();
    if (existing) {
      if (file) payload.profileImage = await UploadService.replace(existing.profileImage, file);
      return (await heroRepository.updateById(existing._id.toString(), payload))!;
    }
    if (file) payload.profileImage = await UploadService.upload(file);
    return heroRepository.create(payload);
  }

  async updateHero(id: string, dto: UpdateHeroDto, file?: Express.Multer.File): Promise<IHeroDocument> {
    ...
    if (file) update.profileImage = await UploadService.replace(existing.profileImage, file);
  ...
  async deleteHero(id: string): Promise<IHeroDocument> {
    ...
    await UploadService.remove(hero.profileImage);
